<?php
    ...
    if(!empty($data['AddNewUser'])){
        ...
        $query_user = "INSERT INTO adms_users (name, email, password, created)
            VALUES ('".$data['name']."', '".$data['email']."', '$password_encrypted', NOW())";
        mysqli_query($conn,$query_user);#executando a query
        if(mysqli_insert_id($conn)){
            $msg="<p style='color:green'>Castrado realizado com sucesso</p>";
        }else{
